      ...
      .then((result) => {
        if (result.success && result.data && result.data.length > 0) {
          const iconMap: Record<string, ReactNode> = {
            "Ruang Kelas": <LayoutDashboard size={28} className="text-white" />,
            "Guru & Staf": <GraduationCap size={28} className="text-white" />,
            "Siswa": <User size={28} className="text-white" />,
            "Siswi": <UserCircle size={28} className="text-white" />,
            "Prestasi": <Trophy size={28} className="text-white" />,
          };
          const dynamicStats = result.data.map((item: any, index: number) => ({
            label: item.label,
            count: item.count,
            icon: iconMap[item.label] || defaultStats[index]?.icon || <Trophy size={28} className="text-white" />,
          }));
          setStats(dynamicStats);
        }
      })
      .catch((err) => console.error("Error fetching statistik from MySQL:", err));
  }, []);
