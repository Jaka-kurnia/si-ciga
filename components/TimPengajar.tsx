    ...
    fetch("/api/pengajar")
      ...
      .then(res => {
        if (res.success && res.data && res.data.length > 0) {
          const head = res.data.find((d: any) => d.isHeadmaster);
          if (head) setHeadMaster(head);
          
          const staffs = res.data.filter((d: any) => !d.isHeadmaster);
          if (staffs.length > 0) setStaffList(staffs);
        }
      })
      .catch(err => console.error("Gagal mengambil data pengajar", err));
  }, []);
