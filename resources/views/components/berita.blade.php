<section id="berita" class="py-20 bg-white relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" data-aos="fade-up" data-aos-delay="200">
        <!-- Tombol Kontrol Kiri -->
        <button id="berita-prev" aria-label="Berita Sebelumnya"
        ...
        <button id="berita-next" aria-label="Berita Selanjutnya"
        ...
        <div class="overflow-hidden px-1 py-4">
            <div id="berita-track" class="flex transition-transform duration-700 ease-in-out -mx-3">
                @foreach ($berita as $item)
                    <div class="berita-card shrink-0 px-3 transition-all duration-300">
                        <div
                            class="bg-white rounded-3xl overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)] transition-all duration-500 border border-gray-100 flex flex-col h-full group">
                            <div class="relative aspect-[16/10] sm:aspect-[4/3] overflow-hidden bg-gray-100">
                                ...
                                <div
                                    class="absolute top-4 left-4 bg-navy/90 text-gold font-poppins text-xs font-bold px-3 py-1.5 rounded-full uppercase shadow-md tracking-wider">
                                    {{ $item->category }}
                                </div>
                            </div>
                            ...
                            <div class="p-6 sm:p-7 flex flex-col flex-1">
                                ...
                                <h3
                                    class="text-lg font-poppins font-bold text-navy mb-3 group-hover:text-gold transition-colors line-clamp-2">
                                    {{ $item->title }}
                                </h3>

                                <p
                                    class="font-montserrat text-gray-600 text-xs sm:text-sm leading-relaxed mb-6 flex-1 line-clamp-3">
                                    {{ $item->excerpt }}
                                </p>

                                <div
                                    class="pt-4 border-t border-gray-100 flex items-center justify-between text-navy font-poppins font-semibold text-xs sm:text-sm group-hover:text-gold transition-colors">
                                    <span>SDN 1 Cigalontang</span>
                                    <span
                                        class="text-gold text-base font-bold transform group-hover:translate-x-1 transition-transform">→</span>
                                </div>
                            </div>
                        </div>
                    </div>
